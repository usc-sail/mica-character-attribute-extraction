"""Create story passages from movie scripts. 
Story passages are texts (20 - 1000 words) containing some character name.
...
Output
------
script passages csv file
    path = mica-character-attribute-extraction/script-passages.csv
    contains imdb-id, passage-id, passage, characters
    passage-id is the position of the passage in the script
...
min_w
    minimum number of words in a passage
max_w
...
# command-line flags
FLAGS = flags.FLAGS
flags.DEFINE_integer("num_characters", default=5, help="number of characters per movie from IMDB cast list")
flags.DEFINE_integer("min_w", default=20, help="minimum number of words in passage")
flags.DEFINE_integer("max_w", default=1000, help="minimum number of words in passage")
...
prompt_file = os.path.join(data_dir, "script-passages.csv")
...
def create_passages(_):
        ...
        if os.path.exists(script_file) and os.path.exists(parse_file) and os.path.exists(imdb_file):
            ...
            while i < len(script):
                if tags[i] == "N":
                    ...
                    if FLAGS.min_w <= len(segment.split()) <= FLAGS.max_w:
    ...
    # passage_tups are tuples: (imdb_id, segment_id, segment, character)
    passage_tups = []
    ...
    for imdb_id in tqdm.tqdm(imdb_ids, unit="movie"):
        ...
        for i, ((segment, _), names) in enumerate(zip(segments_and_docs, segment_names)):
            ...
            for name in imdb_names[:FLAGS.num_characters]:
                name_pattern = re.compile("(^|\W)" + re.escape(name.lower()) + "(\W|$)")
                if re.search(name_pattern, segment.lower()) is not None:
                    prompt_segment_names.add(name)
            if prompt_segment_names:
                passage_tups.append((imdb_id, i, segment, ";".join(sorted(prompt_segment_names))))

    print(f"{len(passage_tups)} (imdb_id, segment, characters) tuples")
    prompt_df = pd.DataFrame(passage_tups, columns=["imdb-id", "passage-id", "passage", "characters"])
